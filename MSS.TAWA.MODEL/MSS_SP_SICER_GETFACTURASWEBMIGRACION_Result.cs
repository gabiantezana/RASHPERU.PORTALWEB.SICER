//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace MSS.TAWA.MODEL
{
    using System;
    
    public partial class MSS_SP_SICER_GETFACTURASWEBMIGRACION_Result
    {
        public int IdFacturaWebMigracion { get; set; }
        public Nullable<int> IdFacturaWeb { get; set; }
        public Nullable<int> Etapa { get; set; }
        public Nullable<int> TipoDocumento { get; set; }
        public string Code { get; set; }
        public Nullable<int> ExCode { get; set; }
        public string CardCode { get; set; }
        public string CardName { get; set; }
        public Nullable<int> Series { get; set; }
        public string CntPerson { get; set; }
        public string DocCurrency { get; set; }
        public Nullable<System.DateTime> DocDate { get; set; }
        public Nullable<System.DateTime> DocDueDate { get; set; }
        public Nullable<System.DateTime> TaxDate { get; set; }
        public string JournalMemo { get; set; }
        public string NumAtCard { get; set; }
        public string ControlAccount { get; set; }
        public Nullable<int> DocEntry { get; set; }
        public string INT_Estado { get; set; }
        public string INT_Error { get; set; }
        public string FolioPref { get; set; }
        public Nullable<int> FolioNum { get; set; }
        public string Asunto { get; set; }
        public Nullable<decimal> DocRate { get; set; }
        public Nullable<int> RendicionesTotales { get; set; }
        public Nullable<int> DocSubType { get; set; }
        public string INT_Ref1 { get; set; }
        public string U_BPP_MDTD { get; set; }
        public string AccountCode { get; set; }
        public string TaxCode { get; set; }
        public Nullable<decimal> LineTotal { get; set; }
        public string CostingCode { get; set; }
        public string CostingCode2 { get; set; }
        public string CostingCode3 { get; set; }
        public string CostingCode4 { get; set; }
        public string CostingCode5 { get; set; }
        public string Description { get; set; }
        public string PaymentMethod { get; set; }
        public Nullable<int> IdFacturaWebRendicion { get; set; }
        public string U_MSS_ORD { get; set; }
    }
}
